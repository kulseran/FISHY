...
class NetServer::Impl {
  ...
  Status start(
    ...
    RET_SM(
        ...
        "Error <" << errno << "> getting address info for server.");
    ...
    if (ret != 0) {
      Log(LL::Error) << "Error <" << errno << "> binding socket.";
      close(m_socket);
      m_socket = INVALID_SOCKET;
  ...
  bool getHostName(std::string &out, const sockaddr_storage &host) {
    ...
    if (getnameinfo(
            (struct sockaddr *) &host, sizeof(host), hostName, NI_MAXHOST, nullptr, 0, NI_NUMERICSERV)
        != 0) {
