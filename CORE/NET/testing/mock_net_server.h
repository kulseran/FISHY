...
class MockNetServer : public ::core::net::iNetServer {
   ...
   */
  virtual Status start(const ::core::net::ServerDef &server) {
  ...
  /**
   * Provide test data for this server to return during the next {@link
   * #update}.
   * @see simulateConnect
   */
  void simulateRecieve(
      const ::core::net::tConnectionId connectionId,
      const std::vector< std::string > &data) {
    std::copy(
        data.begin(), data.end(), std::back_inserter(m_recieved[connectionId]));
  }

  /**
   * @see #simulateRecieve
   */
  virtual Status update(::core::net::iServerConnectionHandler &handler) {
    for (std::vector<::core::net::tConnectionId >::const_iterator itr =
             m_incomingConnections.begin();
         itr != m_incomingConnections.end();
         ++itr) {
      handler.open(*itr);
    }
    ...
    for (std::map<::core::net::tConnectionId, std::deque< std::string > >::
             iterator itr = m_recieved.begin();
         itr != m_recieved.end();
         ++itr) {
      ...
      handler.process(
          *this,
          itr->first,
          ::core::memory::ConstBlob((u8 *) msg.c_str(), msg.length()));
    }
    return Status::OK;
  }
  ...
  virtual Status send(
      const ::core::net::tConnectionId connectionId,
      const ::core::memory::ConstBlob &msg) {
