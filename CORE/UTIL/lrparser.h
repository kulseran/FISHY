...
class LRParser {
  public:
    ...
    class TokenOrNode {
      public:
        ...
        TokenOrNode();
        TokenOrNode(typename Tokenizer< tId >::Token token, const u32 line);
        TokenOrNode(const u32 rule, const u32 line, void *pNode);
    ...
    class RuleOrTokenChain {
      public:
        /**
         * A callback for processing a rule reduction.  This callback is responsible for cleaning
         * up any node pointers returned from the child rules that are passed into it.
         *
         * @param node the output node information from your parser implementation for this rule
         ...
         */
        typedef srutil::delegate<bool(void *&node, const typename tTokenOrNodeList::const_iterator &begin, const typename tTokenOrNodeList::const_iterator &end)> tProc;

        RuleOrTokenChain();
        /**
         * Generates a new RuleOrTokenChain with the given callback.
         * The callback will be processed whenever this chain is used as a reduction rule in the parser.
         *
    ...
    class Rule {
      public:
        ...
        bool reduce(TokenOrNode &node, const u32 chainId, const typename tTokenOrNodeList::const_iterator &begin, const typename tTokenOrNodeList::const_iterator &end) const;
        u32 matchLen(const u32 chainId, const typename tTokenOrNodeList::const_iterator &begin, const typename tTokenOrNodeList::const_iterator &end) const;

        u32 chains() const;
        u32 tokenLen(u32 chain) const;
        u32 getId() const;
  ...
  public:
     ...
     */
    LRParser(const Tokenizer< tId > &tokenizer, const u32 stackDepth = 1024);
     ...
     */
    bool parse(const typename tId::type ignore, const std::string::const_iterator &begin, const std::string::const_iterator &end) const;
     ...
     * @return success status
     */
    bool parse(const typename tId::type ignore, const std::string::const_iterator &begin, const std::string::const_iterator &end, const tErrorCB &cb) const;

  private:
    const Tokenizer< tId > &m_tokenizer;
    const u32 m_stackDepth;
    RuleList m_rules;

    bool reduce(std::vector< TokenOrNode > &stack) const;
    void storeBestError(const std::vector< TokenOrNode > &stack, const tErrorCB &cb) const;
    static void nullErrorCallback(const std::string &logged) {
      Log(LL::Error) << "Parse error: " << logged << std::endl;
    }
};
