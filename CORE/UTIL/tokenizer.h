 ...
 * Splits the input string into {@code Token}s based on a set of {@link RegExPattern}s
 * which define the types of tokens.
 *
 * @template tId the type of the token names must contain members "enum type" and "char *names[]".
 */
...
class Tokenizer {
  public:
    ...
    class Token {
      public:
        Token();
        Token(const typename tId::type id,
     ...
     */
    class const_iterator : public std::iterator< std::forward_iterator_tag, Token > {
      public:
    ...
    /**
     * Scans forward in a string, attempting to match a token starting at {@code begin}.
     *
     ...
     */
    bool getNextToken(Token &token, const std::string::const_iterator &begin, const std::string::const_iterator &end) const;
