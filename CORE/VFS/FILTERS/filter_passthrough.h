...
/**
 * Implementation of {@link streamfilter} that passes through to an underlying
 * delegate.
 */
class passthrough : public streamfilter {
  public:
  passthrough(
      const pos_type low = 0,
      const pos_type len = std::numeric_limits< s64 >::max());
  virtual ~passthrough() {}
  ...
  pos_type m_seekLow;
  pos_type m_seekLen;
  off_type limitReadOff(const off_type sz) {
    return std::min(sz, off_type(m_seekLen - m_curPos));
  }
