...
REGISTER_TEST_CASE(testSuccessfulParse) {
  ...
  tokenTypes.push_back(Tokenizer<eTokenNames>::tTokenizer(eTokenNames::TOKEN_IDENT, RegExPattern("[\\a][\\w]*")));
  tokenTypes.push_back(Tokenizer<eTokenNames>::tTokenizer(eTokenNames::TOKEN_OPERATOR_ASSIGN, RegExPattern("=")));
  tokenTypes.push_back(Tokenizer<eTokenNames>::tTokenizer(eTokenNames::TOKEN_NUMERIC, RegExPattern("[\\d]*(\\.[\\d]+)?")));
  tokenTypes.push_back(Tokenizer<eTokenNames>::tTokenizer(eTokenNames::TOKEN_END_STATEMENT, RegExPattern(";")));
  ...
  for (Tokenizer<eTokenNames>::const_iterator itr = tokenizer.begin(input); itr != tokenizer.end(); ++itr) {
