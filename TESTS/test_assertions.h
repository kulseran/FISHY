 ...
 * test_assertions.h
 *
 * Functions to be called in test cases for checking the validity of tested code's responses.
 */
...
inline void assertEquals(const tTypeT &t, const tTypeU &u, bool(*pred)(const tTypeT &, const tTypeU &) = nullptr) {
  if (pred != nullptr) {
    if (pred(t, u)) {
      return;
    }
 ...
 */
template< typename tTypeT, typename tTypeU >
inline void assertContains(const tTypeT &t, const tTypeU &u) { fail(); }

template<>
inline void assertContains<std::string, std::string>(const std::string &t, const std::string &u) {
...
#define TEST(assertExpr) try { assertExpr; } catch (testing::TestAssertionException &e) { e.addTrace(__FILE__, __FUNCTION__, __LINE__); throw e; }
